document.addEventListener("DOMContentLoaded", () => {
  ...
  addButton.addEventListener("click", (e) => {
    if (bookAuthor.validity.tooShort) {
        bookAuthor.setCustomValidity(`Author must be ${bookAuthor.minLength} characters long.`)
    } 
    if (bookTitle.validity.tooShort) {
        bookTitle.setCustomValidity(`Title must be ${bookTitle.minLength} characters long.`);
    }
    if (bookPages.validity.tooShort || !bookPages.value) {
        bookPages.setCustomValidity(`The book pages must be at least ${bookPages.minLength} digit(s) long.`)
    }
    else {
        const createdBook = new Book(
          bookAuthor.value,
